...
const Magasin = (props) => {
  const {showSuccessToast,showFailedToast, magasin, 
  ...
  const validate = (fieldValues = values) => {
    ...
    if ("label" in fieldValues)
      temp.label = (fieldValues.label && fieldValues.label != "") ? "" : "le nom est requis";
  ...
  const handleSubmit = (event) => {
    ...
    if (validate()) {
      setLoading(true)
      //console.log(values);
      var prod = { ...values, dateCreation : magasin === null ? formatDate(now) : magasin.dateCreation , 
        prixAchat: parseFloat(values.prixAchat), prixVente: parseFloat(values.prixVente)};
      if(magasin === null){
        console.log(prod);
        axios.post(`magasins/add`, prod).then(
          (res) => {
        ...
        axios.put(`magasins/${values.id}`, prod).then(
          (res) => {
